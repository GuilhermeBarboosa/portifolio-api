<?php
	...
	class File {
		...
		public function deleteDoc($path, $from, $to, $name){
			return unlink(str_replace($from, $to, $path) . BAR . RES . BAR . DOC . BAR . $name);
		}
		
		public function deleteImg($path, $from, $to, $name){
			return unlink(str_replace($from, $to, $path) . BAR . RES . BAR . IMG . BAR . LDPI . BAR . $name);
			return unlink(str_replace($from, $to, $path) . BAR . RES . BAR . IMG . BAR . MDPI . BAR . $name);
			return unlink(str_replace($from, $to, $path) . BAR . RES . BAR . IMG . BAR . HDPI . BAR . $name);
		}
